		...
		*stream_out << buf;
		...
		if ( krui_setFirstFTypeSite() )  {
			...
			do  {
				...
				sprintf( buf, " %-s", krui_getFTypeSiteName() );
				*stream_out << buf;
				retchk( stream_out );
				second = TRUE;
			}
...
krui_err  SnnsCLib::krio_writeDefaultDefinitions(void)
{
	...
	*stream_out << buf;
	...
	if ((act_func != NULL) && (out_func != NULL))  {
		sprintf( buf, fmt_shape1, act, bias, getTType( st ),
		...
		if ( no_Ftype )  {
			if ( krui_setFirstSite() )  {
				...
				do  {
					...
					sprintf( buf, " %-s", krui_getSiteName() );
					*stream_out << buf;
					retchk( stream_out );
					second = TRUE;
				}
...
krui_err  SnnsCLib::krio_writeSourcesAndWeights(void)
{
	...
	do  {
		...
		if ((tacoma_mode)&&((val_a != 0.0)&&(val_b != 0.0))) {
			sprintf( buf, fmt_shape4, source_unit, weight, val_b, val_a);
			*stream_out << buf;
		}   else {
			sprintf( buf, fmt_shape3, source_unit, weight );
			*stream_out << buf;
		}
		retchk( stream_out );
...
krui_err  SnnsCLib::krio_writeConnectionDefs(void)
{
	...
	*stream_out << buf;
	...
	do  {
		...
		case  DIRECT_LINKS:
			sprintf( buf, fmt_shape1, target_unit, " ");
			*stream_out << buf;
			retchk( stream_out );
			//err = 
		...
		case  SITES:
			...
			do  {
				if (krui_getFirstPredUnit( &weight ) > 0)  {
					if (second)  {
						sprintf( buf, fmt_shape2, krui_getSiteName() );
						*stream_out << buf;
						retchk( stream_out );
					}
					else  {
	...
	*stream_out << buf;
	...
	for (i = MinUnitNo, unit_ptr = unit_array + MinUnitNo; 
		...
		{				/*  unit is in use and 'fresh' */
			...
			sprintf( buf, fmt_shape1, subnet_no, i);
			*stream_out << buf;
			retchk( stream_out );
			elem_no = 0;
	...
	*stream_out << buf;
	...
	for (i = MinUnitNo, unit_ptr = unit_array + MinUnitNo; 
		...
		{				/*  unit is in use and 'fresh' */
			...
			sprintf( buf, fmt_shape1, layer_no, i);
			*stream_out << buf;
			retchk( stream_out );
			elem_no = 0;
	...
	if (transTableSize > 0)  {
		...
		*stream_out << buf;
		...
		for (z_index = 0; z_index < transTableSize; z_index++)  {
			sprintf( buf, "%8d |", transTable[z_index].x);
			*stream_out << buf;
			retchk( stream_out );
			sprintf( buf, "%8d |", transTable[z_index].y);
			*stream_out << buf;
			retchk( stream_out );
		...
		float *Update_param_array, int NoOfUpdateParam)
{
  ...
  { //  Can't open file
	...
	sprintf( buf, resHeader[0], krio_getIOVersion() );
	*stream_out << buf;
	...
	strcat( work_str, " %s\n" );
	sprintf( buf, work_str, ctime( (time_t *) &clock) );
	*stream_out << buf;
	RETCHKGTO( stream_out );
	lineno++;
	...
	sprintf( buf, work_str, end - start + 1, 
	...
	*stream_out << buf;
	...
	sprintf( buf, work_str, startpattern, endpattern);
	*stream_out << buf;
	RETCHKGTO( stream_out );
	lineno += 2;
